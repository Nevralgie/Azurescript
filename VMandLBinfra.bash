...
echo "Name of the new security group rule ? :"
read MYnsgload
...
# Subnet to use only for Azure Bastion, used by AB to connect to the virtual network
...
# IP used by the user to connect to Azure Bastion (traffic will then be forwarded to your virtual network if the user is authenticated correctly)
echo "Name of your Bastion IP :"
read BIP
az network public-ip create \
    --resource-group $RSG \
    --name $BIP \
...
# Name of your AB host
echo "Bastion host :"
read BHOST
az network bastion create \
    --resource-group $RSG \
    --name $BHOST \
    --public-ip-address $BIP \
    --vnet-name $vnet \
    --location westeurope \
    --no-wait true
